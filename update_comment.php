...
try {
    ...
    // レコードが存在するか確認
    $checkStmt = $pdo->prepare("SELECT * FROM book_clicks WHERE id = ?");
    ...
    // コメントを更新
    $updateStmt = $pdo->prepare("UPDATE book_clicks SET comment = ? WHERE id = ?");
    $result = $updateStmt->execute([$comment, $bookId]);
